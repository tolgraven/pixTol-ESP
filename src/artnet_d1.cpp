...
// HomieSetting<long> universes("universes", "number of DMX universes");
...
// - neopixelbus
//   - RotateLeft/Right ShiftLeft/Right? for direct sweeps/animations of current state however it was reached...
...
// NeoPixelBus<NeoGrbwFeature, NeoEsp8266BitBang800KbpsMethod> *bus = NULL;

// NeoPixelBus<NeoGrbwFeature, NeoEsp8266Dma800KbpsMethod> neoPixelStrip(LED_COUNT); // only works for RX0 (GPIO03) pin
...
void setupOTA();
void sendWS();
void flushNeoPixelBus(uint8_t universe);
...
void setup() {
  Serial.begin(115200);
	...
	// uint8_t universes = (led_count * bytes_per_pixel) / 512;
	universes = 2;
	// Serial.println(universes);
		
	if(bus) delete bus;
	bus = new NeoPixelBus<NeoGrbwFeature, NeoEsp8266Dma800KbpsMethod>(led_count);
	// bus = new NeoPixelBus<NeoGrbwFeature, NeoEsp8266BitBang800KbpsMethod>(led_count, LED_PIN);
	bus->Begin();
	// bus->Show();
...
void logPixels() {
	...
	for(size_t p = 0; p < bufsize; ++p) {
		Serial.printf("%d ", pixels[p]);
	}
	// Serial.flush();
	// Homie.getLogger() << pixels[0] << pixels[1] << pixels[2] << endl;
}
  ...
  if(!udp.parsePacket()) return;
	...
	if(h[0]=='A' && h[1]=='r' && h[2]=='t' && h[3]=='-' && h[4]=='N' && h[5]=='e' && h[6]=='t') {
		if (h[8] == 0x00 && h[9] == ARTNET_DATA && h[15] == net) {
			// if ((h[14] == (subnet << 4) + universe) || (h[14] == (subnet << 4) + universe2)) { // UNIVERSE
			for(int uni = universe; uni <= universes; uni++) {
				// if (h[14] == (subnet << 4) + universe) { // UNIVERSE
...
void flushNeoPixelBus(uint8_t universe) {
	// uint16_t pixel = 0;
	// uint16_t pixel = 100 * (universe - 1);
	// uint16_t pixel = 75 * (universe - 1);
	uint16_t pixel = 128 * (universe - 1);
  for(uint16_t t = 0; t < uniSize; t += bytes_per_pixel) { 	// loop each pixel (4 bytes) in universe
		if(bytes_per_pixel == 3) {
			RgbColor color 	= RgbColor(uniData[t], uniData[t+1], uniData[t+2]);
