...
public:
    ...
    NeoEsp32RmtMethodBase(uint8_t rmtCh, uint8_t pin, uint16_t pixelCount, size_t elementSize = 3)  :
        _rmtCh(static_cast<rmt_channel_t>(rmtCh)), _pin(pin)
    {
    ...
    ~NeoEsp32RmtMethodBase()
    {
        ...
        ESP_ERROR_CHECK_WITHOUT_ABORT(rmt_wait_tx_done(_rmtCh, 10000 / portTICK_PERIOD_MS));

        ESP_ERROR_CHECK(rmt_driver_uninstall(_rmtCh));
    ...
    {
        return (ESP_OK == rmt_wait_tx_done(_rmtCh, 0));
    }
    ...
    void Initialize()
    {
        ...
        ESP_ERROR_CHECK(rmt_driver_install(_rmtCh, 0, 0));
        ESP_ERROR_CHECK(rmt_translator_init(_rmtCh, _translate));
    }
    ...
    void Update(bool maintainBufferConsistency)
    {
        ...
        if (ESP_OK == ESP_ERROR_CHECK_WITHOUT_ABORT(rmt_wait_tx_done(_rmtCh, 10000 / portTICK_PERIOD_MS)))
        {
            // now start the RMT transmit with the editing buffer before we swap
            ESP_ERROR_CHECK_WITHOUT_ABORT(rmt_write_sample(_rmtCh, _pixelsEditing, _pixelsSize, false));
...
private:
    const rmt_channel_t _rmtCh;    // output pin number
